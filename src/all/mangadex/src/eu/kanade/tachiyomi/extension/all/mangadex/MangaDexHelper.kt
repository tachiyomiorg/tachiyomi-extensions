...
class MangaDexHelper {
     ...
     */
    fun getChapterEndpoint(mangaId : String, offset : Int, langCode : String) = "/manga/$mangaId/feed?limit=500&offset=$offset&locales[]=$langCode"

    /**
     * Check if the manga id is a valid uuid
     */
    fun containsUuid(id: String) = id.contains(uuidRegex)
    ...
    internal class TagWithoutId(name: String) : Filter.TriState(name)
    internal class Tag(val id : String, name : String) : Filter.TriState(name)

    internal val demographics = listOf(
        TagWithoutId("Shounen"),
        TagWithoutId("Shoujo"),
        TagWithoutId("Seinen"),
        TagWithoutId("Josei")
    )
