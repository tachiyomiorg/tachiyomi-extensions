package eu.kanade.tachiyomi.extension.it.dgtread
...
class Dgtread : ParsedHttpSource() {

    override val name = "Dgtread"
    ...
    override fun searchMangaRequest(page: Int, query: String, filters: FilterList): Request = throw Exception("La ricerca non è abilitata su questa fonte.")
...
info[chapter]=53
info[ch_sub]=0
info[title]=Digital Team - Act-Age Capitolo 53 Ita

*/

        val body = RequestBody.create(null, "info[manga]=$infomanga&info[chapter]=$infochapter&info[ch_sub]=$infoch_sub&info[title]=$title")
        return client.newCall(POST("$baseUrl/reader/c_i", xhrHeaders, body)).execute().asJsoup().select("body").text()
    }
    ...
    override fun pageListParse(document: Document): List<Page> {
        ...
        Log.e("Dgtread", getXhrPages(script_content, title))
        // POST(baseUrl + SEARCH_URL, headers, buildRequestBody(page = page, sort = 1))
