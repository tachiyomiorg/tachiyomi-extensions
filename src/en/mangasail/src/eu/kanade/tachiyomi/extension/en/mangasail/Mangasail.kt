...
class Mangasail : ParsedHttpSource() {
    ...
    override fun latestUpdatesFromElement(element: Element): SManga {
        ...
        element.select("a:has(img)").let {
            ...
            val coverUrl = it.select("img").first().attr("src").substringBefore("?")
            val coverUrlStart = coverUrl.substringBefore("styles")
            val coverUrlEnd = coverUrl.substringAfter("public/")
            manga.thumbnail_url = "$coverUrlStart$coverUrlEnd"
        }
        return manga
    }
    ...
    private fun getNodeDetail(node: String, field:String): String {
        ...
        if (call.isSuccessful) {
            when (field) {
                "field_image2" -> return parse(call.body()!!.string().substringAfter("en\":\"").replace("\\", ""), baseUrl).select("img").attr("src")
                "field_status", "field_author", "field_artist" -> return parse(call.body()!!.string().substringAfter("en\":\"").replace("\\", ""), baseUrl).select("div.field-item").text()
                "body" -> return parse(StringEscapeUtils.unescapeJson(call.body()!!.string()).substringAfter("en\":\""), baseUrl).select("p").text().substringAfter("summary: ")
                "field_genres" -> return parse(call.body()!!.string().substringAfter("en\":\"").replace("\\", ""), baseUrl).select("a").text()
            }
        }
            return ""
    }
