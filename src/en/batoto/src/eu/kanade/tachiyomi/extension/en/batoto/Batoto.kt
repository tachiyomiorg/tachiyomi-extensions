...
class Batoto : ParsedHttpSource(), LoginSource {
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        val manga = SManga.create()
        manga.author = artistElement.select("td:eq(1)").text()
        manga.artist = artistElement.select("td:eq(2)").text() ?: manga.author
        manga.description = tbody.select("tr:contains(Description:) > td:eq(1)").text()
        manga.thumbnail_url = document.select("img[src^=http://img.bato.to/forums/uploads/]").first()?.attr("src")
        manga.status = parseStatus(document.select("tr:contains(Status:) > td:eq(1)").text())
